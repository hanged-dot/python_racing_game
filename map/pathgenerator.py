...
class PathGenerator:
    ...
    def __is_eligible_point(self, v):
        if self.__parent[v] != -1:
            ...
            while t != -1:
                if (dist_point_segment(self.__points[v], self.__points[p], self.__points[t])
                        <= self.__path_width and dist_to_t > self.__shortening_tolerance * self.__path_width):
                    return False
            ...
            while self.__parent[t] != -1:
                if (dist_point_segment(self.__points[t], self.__points[self.__parent[t]], self.__points[v])
                        <= self.__path_width and dist_to_t > self.__shortening_tolerance * self.__path_width):
                    return False
