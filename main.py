...
opponent_car = AutomatedCar(road,display)
...
while not crash:
    ...
    if opponent_car._AutomatedCar__velocity < 2.0:
        opponent_car.update(True)
    else:
        opponent_car.update(False)
    road.check_boundaries(opponent_car)
    road.check_for_checkpoints(opponent_car)
    
    background.draw()
    road.draw(player_car.get_position())
    player_car.draw(player_car.get_position())
    opponent_car.draw(player_car.get_position())
